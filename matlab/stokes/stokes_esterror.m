function errest = stokes_esterror(z,zpanels,zDrops,Npanels, ...
...
for j=1:numel(z) %Go through all points in domain
    ...
    for k = 1:Npanels %Go through all panels
        ...
        % === Compute estimate
        est1 = abs( 1/(pi*1i) * imag((den_t * knq(n,0,t0))) );
        est2 = 1/(2*pi) * abs( conj(den_t * n_t * knq(n,0,t0)) );
        est3 = 1/(2*pi) * abs( conj(den_t .* knq(n,1,t0)/zp_t * factor) );
        errest(j) = abs(est1 + est2 + est3);
