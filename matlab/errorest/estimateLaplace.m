...
kernel1 = @(x, y, ty) (  ty./ (y-x) );
integrand1 = @(t,z) kernel1(z, g(t), gp(t)).* density(t);

% === Integral 2
kernel2 =  @(x,y,ty) 1./((y-x).^2).*ty;
integrand2 = @(t,z) kernel2(z,g(t),gp(t)).*density(t);

% === Integral 3
kernel3 = @(x,y,ty) conj(y-x)./((y-x).^2).*ty;
integrand3 = @(t,z) kernel3(z,g(t),gp(t)).*density(t);
...
[I1, Q1, est1, I2, Q2, est2, I3, Q3, est3] = deal(zeros(size(Z)));
for j=1:numel(Z)
    z = Z(j);
    ...
    I2(j) = integral(@(t) integrand2(t, z), -1, 1, ...
        'abstol', eps, 'reltol', eps);
    I3(j) = integral(@(t) integrand3(t,z), -1, 1, ...
        'abstol', eps, 'reltol', eps);
    warning on
    
    % === Direct
    Q1(j) = sum(integrand1(t16, z) .* w16);
    Q2(j) = sum(integrand2(t16, z) .* w16);
    Q3(j) = sum(integrand3(t16, z) .* w16); 
...
%     disp('Compute estimate')
%     est1(j) = abs(imag(2*pi*den_t*(t0 + sgn*sqrt(t0.^2-1)).^(-2*n-1)));
%     t0 = abs(real(t0)) + 1i*abs(imag(t0));
    est1(j) = abs((den_t*knq(n,0,t0)));
    est2(j) = abs(den_t.*knq(n,1,t0)/zp_t);
    est3(j) = est2(j)*factor;
